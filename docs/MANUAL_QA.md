...
- [ ] **Usage Access + Overlay grant round-trip on a physical device** — `BlockingPermissionsModule.kt` is real and `./gradlew assembleDebug` builds it successfully, but there's no physical Android device on this machine to install and run it on this phase. On a device: press Allow on Screen 2 → lands on the Usage Access settings screen (`ACTION_USAGE_ACCESS_SETTINGS`) → grant it → back button returns to the app → the `AppState` recheck should silently re-request, land on the Overlay settings screen (`ACTION_MANAGE_OVERLAY_PERMISSION`) next (one grant at a time — there's no combined settings screen) → grant it → return → the recheck should now report `granted` and advance to Home. Also verify: denying either grant and tapping "Open settings" behaves the same way but stays on the denied fallback. **Note (2026-08-15):** this item described the chained round-trip from the day it was written, but the code only gained it in the Phase 3 follow-up below — until then the second permission needed a second Allow press. Read the two items together.

### Phase 3 follow-up — one Allow tap for both grants *(2026-08-15)*

**Run on the Android emulator (API 36, `LockalTime_API36`) on 2026-08-15.** Emulator, not physical hardware — the physical-device item above stays open — but this is a real Android runtime and the three behavioural checks below were driven end to end, from `pm clear` + both grants revoked to both granted.

- [x] **One Allow press carries the user through both Settings screens.** Verified. From a clean state, one tap on Allow opened Usage Access; granting it and pressing back opened Display-over-other-apps **by itself**; granting that and pressing back advanced the app past the permission screen to the auth gate. Allow was pressed exactly once. Confirmed at the OS level rather than by eye: `topResumedActivity` moved `MainActivity` → `Settings$AppUsageAccessSettingsActivity` → `Settings$SpaActivity` → `RequestIgnoreBatteryOptimizations`. That last one is decisive — the battery-optimization ask only fires from `handleGranted()`, so the app really did resolve `granted`.
- [x] **Backing out does not trap the user in Settings.** Verified, and this is the check that mattered. Backed out of the Overlay screen granting nothing, then returned to the app: it stayed on `MainActivity` for the full observation window and never reopened Settings. No loop.
- [x] **A deliberate retry after backing out still works.** Verified — pressing Allow after a back-out opened the Settings screen normally rather than being refused.
- [ ] **Re-run the back-out guard against a *surviving* JS screen.** The 2026-08-15 run backed out far enough to reach the launcher, and the priming screen was recreated on return (it came back in the `priming` state, not `denied`), so the re-drive path was **not** what got exercised — the guard was verified, but via the weaker "screen was rebuilt, nothing pending" route. Redo it leaving the app's own task intact (recents switch, or a single back from the Settings screen) so the screen survives, `awaitingSettingsReturn` is still true, and the native refusal is the thing that stops the loop. Expected then: the **denied/recovery** state, still no relaunch.
- [x] **Where the `package:`-scoped intent lands on API 36 — answered, and the two actions differ.** `ACTION_USAGE_ACCESS_SETTINGS` honours the URI: it opens `Settings$AppUsageAccessSettingsActivity`, Lockal Time's own single-toggle screen ("App usage data → Permit access to app usage data"). **`ACTION_MANAGE_OVERLAY_PERMISSION` does not**: fired by the app itself with the same `package:` URI, it opens the full per-app "Display over other apps" list, where the user has to find LockalTime among Android Auto, Google, Play services, Phone, Photos, Voice Access and YouTube. That list is the screen the owner objected to on 2026-08-15, and it appears to be OS behaviour rather than a missing URI on our side — the code has always passed it. See the backlog follow-up; no fix is known yet, and forcing it may not be possible.

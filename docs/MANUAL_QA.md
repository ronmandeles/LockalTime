...
### Phase 3 follow-up — one Allow tap for both grants *(2026-08-15, never run)*

- [ ] **One Allow press carries the user through both Settings screens.** Fresh install, neither permission granted. Press Allow once and do not press it again: Usage Access opens → grant → back → **Overlay should open by itself** → grant → back → the screen advances to Home. The whole point of the change is that the second screen arrives without a second tap; if it doesn't, the foreground re-drive isn't firing.
- [ ] **Backing out does not trap the user in Settings.** The loop guard, and the thing most worth testing, because getting it wrong makes the app unusable rather than merely annoying. Press Allow, then leave the Usage Access screen with **back, granting nothing**. Expected: the app shows the denied/recovery state and stays there. It must **not** immediately reopen Settings — if it does, every return bounces the user straight back out and there is no way into the app short of force-stopping it. Repeat for the Overlay screen (grant Usage Access, back out of Overlay).
- [ ] **A deliberate retry after backing out still works.** Continuing from the state above, press "Open settings" / Allow again — the sequence should start over and open a Settings screen normally, not answer denied a second time. (`pendingPermission` is cleared on the refusal so the next deliberate ask isn't punished for the previous back-out.)
- [ ] **Where the `package:`-scoped intent actually lands on API 36 — open question, not a fix.** Both intents carry `data = Uri.parse("package:com.lockaltime.app")`, which is meant to open Lockal Time's *own* toggle rather than the full per-app list. A screenshot from the 2026-08-15 emulator run showed the full "Display over other apps" list, but that is not conclusive — the user may simply have navigated back to it. Check where each screen actually opens. If it is the full list, the host has to hunt for "LockalTime" among every app on the device, and this is worth its own fix; if it is the app's own toggle, there is nothing to do.

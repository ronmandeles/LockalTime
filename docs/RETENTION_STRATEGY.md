...
### 4. Social proof / leaderboards / comparison — **Adopted (leaderboard + mutual friends), Phase 6.5**
Comparison against others is a strong driver (visible leaderboards, "5 friends completed a session today"). Deferred out of Phase 5 for scope reasons only (no friend graph, no privacy design) — `ARCHITECTURE.md` §9 originally rejected this on anti-comparison-anxiety grounds, and this analysis's own first pass on Phase 6.5 defaulted to no leaderboard for that reason. **The owner explicitly reversed that at the plan-approval step**, asking for a friends-only leaderboard ranked by total lifetime points — a real, deliberate reopening of a previously-rejected pattern, not an oversight. Implemented Phase 6.5: mutual request/accept friendship (never a one-directional follow that lets someone watch a stranger), discovery by username (a new column, the owner's explicit choice over a shareable code or email search), and a ranked leaderboard showing only `total_points` and a coarse `hadSessionToday` signal per friend — never the real `current_streak` or raw session history. The anxiety concern this section originally raised is addressed structurally, not by avoiding leaderboards altogether: comparison is opt-in (both parties consented by becoming friends) and scoped to friends only, never a global or stranger-visible ranking.
...
None remaining — both items previously listed here (push infrastructure, social graph/comparison surfaces) closed in Phases 5.5 and 6.5 respectively.
...
- Social graph + comparison surfaces (mutual friends, username search, a friends-only points leaderboard) — Phase 6.5, see `backlog.md` and `docs/ARCHITECTURE.md` §9.

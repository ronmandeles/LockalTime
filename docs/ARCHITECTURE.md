...
**Deferred (good fit, blocked on missing infrastructure — see `backlog.md`):** push notifications (streak-at-risk, session invites — no `device_tokens` table or send pipeline exists), social proof / leaderboards (no friend graph or privacy design exists).

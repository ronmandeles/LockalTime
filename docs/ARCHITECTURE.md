...
**Implemented Phase 2 task 2.7** (skeleton — no native blocking, sessions are "virtual"): Screens 4–8 all real navigator routes (`apps/mobile/src/navigation/types.ts`). Screen 7's QR Scan is manual token entry only this phase (no camera dependency — see `services/qr-scanner.ts` and `docs/MANUAL_QA.md`); the actual `POST /sessions/join` call happens on Screen 8 (pre-join confirmation), not Screen 7, so a bad/expired/full code surfaces its specific error where the user can act on it. The edge-case screens listed above (QR expired/invalid/at-capacity) are *not* separate screens — they're error states rendered inline on Screen 8, keyed off the join outcome's failure code.
